...
from rpy2.robjects import r, pandas2ri, NA_Character
pandas2ri.activate()
...
def setdataframe(file1,file2,outname="",
                 SP1='Human',SP2='Swine',
                 delete_=[],include_=[], disable_na = False, multisample = 0,
                 maxSamples = None):
    ...
    if D1.shape[0] > D2.shape[0]:
        nD1 = D1.sample(n=D2.shape[0])
        nD2 = D2
    elif D1.shape[0] < D2.shape[0]:
        nD2 = D2.sample(n=D1.shape[0])
        nD1 = D1
    elif D1.shape[0] == D2.shape[0]:
        nD2 = D2
        nD1 = D1

    limiter = min(D1.shape[0], D2.shape[0])

    if multisample > 0:
        for i in range(multisample-1):
            nD1 = pd.concat([nD1, D1.sample(n=limiter)], ignore_index = True)
            nD2 = pd.concat([nD2, D2.sample(n=limiter)], ignore_index = True)
    if maxSamples is not None:
        if nD1.shape[0] > maxSamples:
            nD1 = nD1.sample(n=maxSamples)
        if nD2.shape[0] > maxSamples:
            nD2 = nD2.sample(n=maxSamples)
    D1 = nD1
    D2 = nD2
    ...
    datatrain_indices = list(datatrain.columns.values)

    dtrx, dtry = datatrain.shape

    if not disable_na:
        for x in datatrain_indices:
            if re.match(posregex, x):
                for j in range(dtrx):
                        if datatrain[x][j] == 'X' or datatrain[x][j] == '-':
                            datatrain[x][j] = NA_Character
                            datatrainNA += 1
...
def tree_export(TR,outfilename='out.dot',
    ...
    if TYPE=='ortho':
        LABELTYPE='xlabel'
    out_file = open(os.path.join(output_dir, outfilename), "w")
    ...
    if EXEC:
        outfilename_=os.path.join(output_dir,"TREE_"+outfilename.replace('.dot','.png'))
        subprocess.Popen(["dot", '-Tpng', os.path.join(output_dir, outfilename), '-o', outfilename_])
    return
